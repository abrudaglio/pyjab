...
class WideStringOffsetConverter:
    ...
    def __init__(self, text: str):
        ...
        self.encoded: bytes = text.encode(
    ...
    def strToWideOffsets(
        ...
        if strStart == 0:
            ...
            precedingBytes: bytes = self.decoded[:strStart].encode(
                self._encoding, errors="surrogatepass")
            wideStringStart = len(precedingBytes) // self._bytesPerIndex
        if strStart == strEnd:
            return (wideStringStart, wideStringStart)
        encodedRange: bytes = self.decoded[strStart:strEnd].encode(
            self._encoding, errors="surrogatepass")
        wideStringEnd: int = wideStringStart + \
            (len(encodedRange) // self._bytesPerIndex)
    ...
    def wideToStrOffsets(
        ...
        if wideStringStart < 0 or wideStringStart > self.wideStringLength:
            ...
            wideStringStart = max(
                0, min(wideStringStart, self.wideStringLength))
        ...
        precedingStr = self.encoded[:bytesStart].decode(
            self._encoding, errors="surrogatepass")
